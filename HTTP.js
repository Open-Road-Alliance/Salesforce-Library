...
const fetch_ = ({ batch = false, method = METHODS.GET, queryParameters = {}, payload = {}, sObject = '', sObjectId = '', apiVersion = 50 }) => {
  // const fetch_ = (config) => {
  try {
    ...
    const endpoint = createEndpoint_({ batch, method, sObject, sObjectId, apiVersion, queryParameters });
...
const createEndpoint_ = ({ batch, method, queryParameters, sObject, sObjectId, apiVersion }) => {
  ...
  switch (method) {
    case METHODS.GET:
      ...
      break;
    case METHODS.PATCH:
      endpoint += (batch ? 'composite/tree/' : 'sobjects/') + sObject + '/' + sObjectId + '.json';
      break;
    case METHODS.POST:
...
const getOptions_ = ({ payload, accessToken, method, contentType = 'application/json' }) => {
  const headers = {
...
const createSoqlQuery_ = (queryParameters) => {
  ...
  const clauses = ['SELECT', 'FROM', 'WHERE', 'GROUP BY', 'ORDER BY', 'LIMIT', 'OFFSET', 'HAVING'];
  let query = '';
